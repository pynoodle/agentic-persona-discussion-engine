...
│       │                                                  │
│       ▼                                                  │
│  GPT 감성 분석 + 전환 의도 스코어링 (0.0~1.0)             │
│       │                                                  │
│       ▼                                                  │
│  구조화된 리뷰 JSON (2,621개) → LangChain RAG 인덱싱      │
└────────────────────────┬────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────┐
│              AutoGen Multi-Agent Layer                   │
│                                                          │
│  Facilitator Agent (패시브 조율자)                       │
│       │                                                  │
│       ├── Customer Agents (4명)                          │
│       │   ├── iPhone→Galaxy 전환자  (570명 데이터 기반)   │
│       │   ├── Galaxy 충성 고객      (110명 데이터 기반)   │
│       │   ├── 기술 애호가           (데이터 기반)         │
│       │   └── 가격 민감 고객        (데이터 기반)         │
│       │                                                  │
│       └── Employee Agents (3명)                          │
...
│                                                          │
│  DebateSystem (기본) / DeepDebateSystem (5-phase)        │
│       │                                                  │
│       ▼                                                  │
│  VotingSystem (1~5점 투표 → 가중 평균 → 가결/부결)        │
│       │                                                  │
│       ▼                                                  │
│  JSON 스트림 출력 (start/message/vote/complete)           │
│       │                                                  │
│       ▼                                                  │
